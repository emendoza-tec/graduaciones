import { ElementRef, AfterViewInit, Directive, Host, Optional, Renderer2, Self, ViewContainerRef, Input } from "@angular/core";
...
export class PaginatorDirective {
    ...
    private buildPageNumbers() {
        ...
        const containerPaginator = this.vr.element.nativeElement.querySelector(
        ...
        //Agregado de clases
        containerPaginator.classList.add('row');
        paginatorPageSize.classList.add('col');
        paginatorPageSize.classList.add('justify-content-end');
        actionContainer.classList.add('col');
        actionContainer.classList.add('justify-content-center');
        paginatorRangeLabel.classList.add('col');
        paginatorRangeLabel.classList.add('justify-content-start');

        this.ren.removeChild(actionContainer, paginatorRangeLabel);
        this.ren.removeChild(containerPaginator, paginatorPageSize);
                ...
                for (let i = 0; i < nodeArray.length; i++) {
                    if (nodeArray[i].nodeName === "BUTTON") {
                        if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
                            this.ren.setStyle(nodeArray[i], "color", "#A2ADBA");
                            this.ren.setStyle(nodeArray[i], "margin", ".5%");
                        } else if (
                            nodeArray[i].innerHTML.length > 50 &&
                            !nodeArray[i].disabled
                        ) {
                            this.ren.setStyle(nodeArray[i], "color", "#2D3649");
                            this.ren.setStyle(nodeArray[i], "margin", ".5%");
                        } else if (nodeArray[i].innerText !== "..." && nodeArray[i].disabled) {
                            this.ren.setStyle(nodeArray[i], "background-color", "#5D5FEF");
                            this.ren.setStyle(nodeArray[i], "color", "white");
                        } else if (nodeArray[i].innerText === "...") {
                            this.ren.setStyle(nodeArray[i], "background-color", "white");
                            this.ren.setStyle(nodeArray[i], "color", "");
                        }
                    }
                }
            });
        ...
        this.ren.insertBefore(containerPaginator, paginatorRangeLabel, actionContainer);
        this.ren.appendChild(containerPaginator, paginatorPageSize);
    }
    ...
    private createButton(i: any, pageIndex: number): any {
        ...
        this.ren.setStyle(linkBtn, "border", "0px");
